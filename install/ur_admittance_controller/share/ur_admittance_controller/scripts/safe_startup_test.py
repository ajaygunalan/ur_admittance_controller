...
This script tests the safe startup sequence of the admittance controller by:
1. Monitoring pose errors during startup trajectory
2. Verifying errors stay within safety thresholds
3. Reporting test success/failure

The test ensures the robot can safely transition from any initial pose
to the desired start pose without excessive position/orientation errors.

Usage:
    ros2 run ur_admittance_controller safe_startup_test.py
    
Parameters:
    max_error_threshold: Maximum allowed position error (meters)
    test_timeout: Maximum time for test completion (seconds)
    service_timeout: Timeout for service calls (seconds)
...
class SafeStartupTest(Node):
    """
    ROS2 node for testing safe startup of the admittance controller.
    
    This node monitors pose errors during startup and verifies they
    remain within acceptable bounds.
    """
    
    def __init__(self):
        """Initialize the safe startup test node."""
    ...
    def pose_error_callback(self, msg: Twist) -> None:
        """
        Callback for pose error messages.
        
        Tracks maximum error and logs warnings if approaching threshold.
        
        Args:
            msg: Twist message containing pose error (linear = position, angular = orientation)
        """
        ...
        with self.lock:
            ...
            # Warn if error approaches threshold
    ...
    def reset_test_state(self) -> None:
        ...
        Clears all recorded errors and flags.
        """
        with self.lock:
            self.max_error = 0.0
    ...
    def wait_for_service_ready(self) -> bool:
        """
        Wait for the move_to_start_pose service to become available.
    ...
    def test_startup(self) -> bool:
        """
        Execute the complete safe startup test sequence.
        
        This method:
        1. Resets test state
        2. Waits for service availability
        3. Calls move_to_start_pose service
        4. Monitors errors during movement
...
def main(args=None) -> None:
    """
    Main entry point for the safe startup test.
