// wrench_node.cpp  — cleaned, concise, and functionally equivalent (SENSOR→PROBE; no PROBE→BASE)
...
: Node("wrench_node"),
  ...
  // Cache static SENSOR→PROBE once; fail-fast if absent
  if (!tf_buffer_->canTransform(frames::PROBE_FRAME, frames::SENSOR_FRAME,
                                tf2::TimePointZero, std::chrono::seconds(5))) {
    RCLCPP_FATAL(get_logger(), "Static transform %s ← %s unavailable at startup",
                 frames::PROBE_FRAME, frames::SENSOR_FRAME);
    std::exit(1);
  }
  T_SP_ = tf_buffer_->lookupTransform(frames::PROBE_FRAME, frames::SENSOR_FRAME, tf2::TimePointZero);
...
void WrenchNode::WrenchCallback(const WrenchMsg::ConstSharedPtr msg) {
  ...
  // 6) Minimal, useful logging (throttled)
